...
public class JSPHomeController {
	 ...
	 *	- 스크립틀릿 코드 블록은 "<% 와 %>" 사이, 그리고 "<%=%>" 사이에 자바 코드를 포함할 수 있다.
	 *	- 현재는 스크립틀릿을 사용하기 보다는 JSTL과 같은커스텀 태그 라이브러리와 EL을 조합해서 JSP를 구현하는 방법을 많이 사용한다.
	 *
	 * 3. 커스텀 태그 라이브러리
	 * 	- 스크립트 요소가 많아지면 많아질수록 JSP 코드는 복잡해진다.
	 * 	이 문제를 해결하는 한 가지 방법은 커스텀 태그를 사용하는 것이다.
	 *	커스텀 태그를 사용하면 뷰를 표시하기 위한 로직을 공통화하거나, 표현하기 복잡한 로직을 캡슐화 할 수 잇어서 JSP의 구현 코드를 간결하게 만들 수 있다.
	...
	public String home0104(Model model) {
		...
		memberMap.put("userName", "hong");
		
		// memberMap 이라는 키를 정확하게 명시했기 때문에, jsp 페이지에서 명시된 키로 데이터를 출력할 수 있다.
