...
public class TransactionController {
	 ...
	 * 		- 격리성(Isolation)
	 * 			> 트랜잭션으로 처리되는 동안에 외부에서의 간섭이 없어야 한다.
	 * 		- 영속성(Durability)
	 ...
	 * 			클래스나 메서드에 대해 @Transaction 어노테이션을 부여하여 트랜잭션을 적용할 수 있다.
	 * 		
	 * 		서비스 구현 객체에 트랜잭션 적용
	 ...
	 * 		이럴땐 되고 저럴땐 되고가 아니라, 기본적인 스프링 트랜잭션의 정책이 있고 그 안에서 정책 이 외의 옵션으로 특정 에러가 발생했을 때
	 * 		롤백을 진행할 수 있도록 제공한다. (많은 블로그들을 100% 믿지 말기)
	 * 	
	 ...
	 * 					|	Checked Exception			|	Unchecked Exception
 	 ...
 	 *		예외 확인 시점	|	컴파일 단계에서 컴파일이 되지 않는다.	|	런타임 중 예외가 확인된다.
 	 ...
 	 *			- 해당 어노테이션이 클래스 수준에서 선언되면 선언 클래스 및 해당 하위 클래스의 모든 메소드에 기본값이 적용된다.
 	 *			- RuntimeException계열, Error 예외에 대해서는 Rollback이 가능하다.
 	 *		
 	 *			1) isolation (격리수준)
 	 *				- 각 트랜잭션이 존재할 때, 트랜잭션들 기리 서로 고립된 수준을 나타내며 서로간에 가용된 데이터를 컨트롤 할지에 대한 부분들을 설정할 수 있다.
 	 *				- isolation 기본 값은 DEFAULT이다.
 	 *				- 새롭게 시작된 트랜잭션에만 적용되므로, Propagation.REQUIRED 또는 Propagation.REQUIRES_NEW와 함께 사용되도록 독점 설계 되었다.
 	 *				
 	 *				1-1) 옵션
 	 *				
 	 *				** 용어
 	 ...
 	 *									Non-reapeatable Read 및 Phantom Read가 발생할 수 있다.
	 ...
	 *				- 추가적인 트랜잭션을 기존 트랜잭션에 포함 시켜 함께 처리할 수도 있고, 추가적인 트랜잭션처럼 별도의 트랜잭션으로 추가 할 수도 있고, 다른 트랜잭션처럼 진행되다 에러를 발생 시킬수도 있다.
	 *			
	 *				2-1) 옵션
	 *
	 *				- REQUIRED : 현재 트랜잭션을 지원하고 존재하지 않는 경우 새 트랜잭션을 만듭니다. >propagation 기본 default 옵션에 해당합니다.
	 *					>부모/자식간에 상관관계에서 자식부분의 트랜잭션이 rollback 처리 시, 부모까지 영향이 가서 rollback 처리 된다. REQUIRES_NEW : 새로운 랜잭션을 생성한다.
	 *					> rollback은 각각 이루어진다.
	 ...
	 * 						> 읽기 전용에 대한 힌트를 분석할 수 없는 트랜잭션의 경우 throw를 던지지 않고 조용히 힌트를 무시한다.
	 * 
	 * 				4) timeout (트랜잭션 제한시간)
	 *					- 기본값은 -1로 무제한입니다.
