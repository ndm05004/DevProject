...
public class validationController {
	 ...
	 * 
	 * 1. 입력값 검증
	 * - 스프링 MVC Bean Validation 기능을 이용해 요청 파라미터 값이 바인딩된 도메인 클래스(또는 커멘드 클래스)의 입력값 검증을한다.
	 * 
	 * [환경설정]
	 * # 의존 관계 정의
	 ...
	 * - 이떄, 규칙을 활성화하기 위해서 사용할 어노테이션이 있다.
	 * 		> @Validated를 지정한다.
	...
	@RequestMapping(value="/registerValidationFormResult", method = RequestMethod.POST)
	...
	public String registerValidationForm02Result(@Validated Member member, BindingResult result) {
		...
		if(result.hasErrors()) {
			...
			log.info("allErros.size() : " + allErros.size());
			log.info("globalErros.size() : " + globalErros.size());
	 ...
	 * 		@Pattern(regexp=)          : CharSequence는 지정된 정규식과 일치해야하고, 정규식은 Java정규식을 따른다.
	 ...
	 * 		- 테스트 URL: /validation/registerValidationForm02로 요청해서 테스트 진행
	 * 	
	 * 	4. 중첩된 자바빈즈 입력값 검증
