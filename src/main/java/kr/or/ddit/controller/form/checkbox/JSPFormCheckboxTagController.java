...
@Controller
@RequestMapping("/formtag/checkboxe")
@Slf4j
public class JSPFormCheckboxTagController {
	...
	public String registerForm02(Model model, Member member) {
		...
		if(hobbyArray !=null) {
			for(int i=0; i< hobbyArray.length; i++) {
				log.info("member.hobbyArray["+i+"] : " + hobbyArray[i]);
			}
		}else {
		...
		if(hobbyList !=null) {
			for(int i=0; i< hobbyList.size(); i++) {
				log.info("member.hobbyList["+i+"] : " + hobbyList.get(i));
			}
		}else {
			log.info("hobbyList is null");
		}
