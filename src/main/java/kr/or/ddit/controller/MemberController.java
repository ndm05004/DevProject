...
public class MemberController {
	...
	/*
	 * 
	 *  2) URL 경로 상의 경로 변수로부터 데이터를 취득할 수 있다.
	...
	public String register01(String userId, String coin) {
	...
	// 4) HTML Form 필드가 여러개일 겨웅에도 컨트롤러 매개변수명이 일치하면 요청 데이터를 취득할 수 있다.
	@RequestMapping(value="/register02", method = RequestMethod.POST)
	public String register02(String userId, String password, int coin) {
		log.info("register02() 실행...! ");
		log.info("userId:" + userId);
		log.info("password:" + password);
		log.info("coin:" + (coin-10));
		return "success";
	}
		
	
	// 5) HTML Form 필드가 여러개일 경우에 컨트롤러 매개변수의 위치는 상관없이 있는가?
	@RequestMapping(value="/register03", method = RequestMethod.POST)
	public String register03(String userId, String password, int coin) {
	...
	// 6) HTML Form 필드값이 숫자일 경우에는 컨트롤러 매개변수 타입이 숫자형이면 숫자로 타입변환하여 데이터를 취득하는가? 
	...
	// 데이터를 받는 필드명은 동이랗게 userId로 하되, 사용하는 변수명은 username으로 달리 설정할 수 있따.
	...
	// 2) 폼 텍스트 필드 요소값을 자바빈즈 매개변수와 기본 데이터 타입인 정수 타입매개변수로 처리한다.
	...
	public String regsterUserId(String userId) {		
		log.info("regsterUserId() 실행...!");
		log.info("UserId " + userId);
		return "success";
	}
	...
	public String registerSelect(String nationality) {
		...
		log.info("nationity : " + nationality);
		return "success";
	}
	
	
	// 6) 복수 선택이 가능한 폼 셀렉트 박스 요소 값을 기본 데이터 타입인 문자열 타입 매개변수로 처리한다.
	@RequestMapping(value="/registerMultiSelect", method = RequestMethod.POST)
	...
	//8) 복수 선택이 가능한 폼 셀렉트 박스 요소값을 문자열 배열 타입 매개변수로 처리한다.
	@RequestMapping(value="/registerMultiSelect03", method=RequestMethod.POST)
	public String registerMultiSelect03(ArrayList<String> carList) { // 배열형태로는 받아오지만 리스트로는 데이터를 받아올 수 없음
	...
	// 받는 타입을 List로 하게 되면 No primary or default constructor for interface java.util.List 에러 발생
	// 스프링에서 List 타입으로 데이터를 받는데에 문제가 있다. (데이터 바인딩이 안됨)
	// 리스트와 같은 형태의 값을 받으려면 String[]로 여러 데이터를 받아서 List에 담는 방법이나,
	// 객체를 이용하여 데이터를 바인딩 방법이 있다.
	...
	public String registerCheckbox04(String developer) {
		// 값이 존재하면 value 속성안에 들어있는 값이 넘어오고
	...
	// 결과로 Date 타입의 값을 받기 위해서는 default인 yyyy/mm/dd의 형태로 값을 설정해서 보내야 한다.
	// 내가 원하는 Date타입의 형식이 존재한다면 @DateTimeFormat pattern 속성을 이용하여
	// 원하는 패턴ㅇ르 설정하여 날짜 형식의 값을 넘겨받는다(pattern="yyyyMMdd", pattern="yyyy-MM-dd")
	...
	public String registerUser(MemberAllForm member, Model model) {
		...
		if(nationality.length > 0) {
			log.info("member.getNationality.length" +  nationality.length);
			for(int i =0; i<nationality.length; i++){
				log.info("member.getNationality : " +  nationality[i]);
			}
		}else {
		...
		if(cars.length > 0) {
			log.info("member.cars.length" +  cars.length);
			for(int i =0; i<cars.length; i++){
				log.info("member.cars : " +  cars[i]);
			}
		}else {
		...
		if(hobby.length > 0) {
			log.info("member.hobby.length" +  hobby.length);
			for(int i =0; i<hobby.length; i++){
				log.info("member.hobby : " +  hobby[i]);
			}
		}else {
		...
		log.info("member.getAddress().postCode : " + address.getPostCode());
		log.info("member.getAddress().location : " + address.getLocation());
		...
		log.info("card.getIntroduction : " + member.getIntroduction());
	...
	// 6) 여러개의 파일업로드를 폼 파일 요소값을 MultipartFile 타입의 요소를 가진 리스트 컬렉션 타입매개변수로 처리한다. - 불가능
	...
	public String registerFile07(MultiFileMember multipartMember) {
		log.info("registerFile07실행...!");
		List<MultipartFile> pictureList = multipartMember.getPictureList();
	...
	// 8) 파일업로드 폼 파일 요소값과 텍스트 필드 요소값 MultipartFile 타입의 배열 매개변수로 처리한다.
