...
public class DatabaseConnectController {
	 ...
	 *  
	 *  5. 스프링 JDBC
	 *   - SQL로만 데이터베이스를 쉽게 처리할 수 있도록 도와주는 JDBCTemplate 클래스를 제공하낟.
	 *   	
	 ...
	 *   		
	 *   		spring-jdbc 의존관계 설정
	 *   		> pom.xml에 spring-jdbc 의존관계 설정한다.
	 ...
	 *   	3. 게시판 구현
	 *   		
	 *   		기본적인 JDBC를 이용한 디비 설계는 DAO란 sql 쿼리가 노출되어 개발하는 방식이다 보니 우리가 중급 프로젝트때 또는 JSP에서
	 *   		preparestatement방식의 데이터베이스 연동과 다를바 없는 형태로, 게시판을 구현을 진행하는 내용만 간략히 보고 넘어가도록 한다.
	 *   
	 *   	
	 *   
	 ...
	 *   	- EntityManager에서 제공하는 Entity 조작 API를 이용해 Entity에 대해 CRUD 작업을 진행할 수 있다.
	 *   	
	 *   	# Entity 상태
	 *   	- new 상태
	 *   	- 관리 상태
	 ...
	 *   
	 *   	# JPA 관련 어노테이션
	 *   	자바빈즈 클래스 객체가 곧 테이블 구조와 같은 형태이기 때문에, VO클래스 내 @Entity에노테이션을 통해 해당 객체가 테이블과 같은 Entity임을
	 *   	설명하고 @Table어노테이션으로 테이블명을 설정해주면 VO클래스는 데이터베이스의 생성되어 있는 테이블과 연결되기 위한 준비가 된다.
	 *   	ex)
	 *   		@Entity
	 *   		@Table(name="board")
	 *   		public class Board{...
	 *   	그리고 각 필드(멤버변수)에 @Column(name="board_no") 어노테이션으로 테이블 컬럼과 매핑 설정을 한다.
	 *   	설정된 Entity들을 이용하여 JPA객체를 통해서 함수를 호출하면(persist, find 등등) 호출된 함수를 통해 Spring Data JPA가 자동으로
	 *   	Entity를 분석하여 함수에 따른 쿼리를 자동생성하여 결과를 실행, 조회, 수정 등을 처리해준다.
	 ...
	 *   	그리고, 사용하는 데이터베이스 종류가 변경된다 하더라도 문제가 없다. (JPA는 SQL을 직접 건들이지 않고 함수와 같은 기능등로만 쿼리를 요청하고 데이터베이스와
	 *      통신하기 때문이다.)
	 *   	
	 *   	JPA는 JPA의 구현체인 hibernate를 이용하여 사용한다. 그러기 위해선 관련 의존관계 등록 후 사용해야 한다.
	 *   	SQL문을 이용하여 데이터베이스를 연동하여 데이터를 가공하는게 아닌, method를 통해 데이터베이스를 조작할 수 잇다는 장점을
	 *   	이용하여 비지니스 로직을 구성하는데에만 집중할 수 있다. 하지만, 프로젝트의 규모와 복잡도가 클 때는 설계가 잘 못 되는 경우 해당 프로젝트의
	 ...
	 *   	- 예제로 mapper패키지안에 BoardMapper.xml을 구성하여 테스트를 진행한다.
	 *   
	 *   	
	 *   	
	 *   	
