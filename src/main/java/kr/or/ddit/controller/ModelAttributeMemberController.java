...
public class ModelAttributeMemberController {
	...
	// 1) 기본자료형을 매개변수로 선언했을때 전달되는지 확인
	@RequestMapping(value="/register01", method = RequestMethod.POST)
	public String register01(String userId, String password) {
		log.info("register01() 실행..1");
		log.info("userId : " + userId);
		log.info("password : " + password);
		return "result";
	}
	
	// 2) 기본자료형인 매개변수에 @ModelAttribute 어노테이션을 지정하여 데이터를 전달한다.
	@RequestMapping(value="/register02", method = RequestMethod.POST)
	public String register02(
			@ModelAttribute("userId") String userId, 
			@ModelAttribute("password") String password) {
		log.info("register02() 실행..1");
		log.info("userId : " + userId);
		log.info("password : " + password);
