...
public class BoardController {
	 ...
	 *  
	 *  - 해당 경로로 요청 시, board폴더에 read.jsp를 찾아서 페이지를 출력해주는 것 같지만,
	 *  경로상으로만 보면 board/read/boardNo라는 파일까지 찾게 된다.
	 *  
	 *  - URL 경로 상 변하는 값을 경로 변수로 취급한다.
	 ...
	 * 화면으로 응답하는 경우에는 HTTP GET방식과 POST방식 두 가지를 사용할 수 있다.
	 *  
	 */
	
	/*
	 * @RequestMapping(value="/formHome", method = RequestMethod.GET) public String
	 * formHome() { return "formHome"; }
	 * 
	 * // register 경로의 GET방식
	 * 
	 * @RequestMapping(value="/register", method = RequestMethod.GET) public String
	 * resigerForm1() { logger.info("regosterForm() 실행...!"); return "success"; }
	 * 
	 * // register경로의 POST 방식
	 * 
	 * @RequestMapping(value="/register", method = RequestMethod.POST) public String
	 * resiger() { logger.info("regosterForm() 실행...!"); return "success"; }
	 * 
	 * // modify 경로의 GET 방식
	 * 
	 ...
	 * modify() { logger.info("modifyForm() 실행...!"); return "success"; }
	 * 
	 * //remove 경로에 POST 방식
	 * 
	 ...
	 *  버튼이나 링크에 따라 호출할 메소드는 바꿔야 할 때 사용한다.
	 * 
	...
	// /board/get 경로, POST방식, "register" 요청 파라미터에 대한 처리
	...
	public String ajaxHome() {
		logger.info("_ajaxHome() 실행....!");
		return "ajaxHome";
	}
	
	// headers 매핑이지만, headsers 설정을 하지 않고 요청
	...
	/*
	 *  6. Content Type 매핑
	 *  - 요청의 Content Type 페더 값을 매핑 조건으로 지정하는 경우에는 consumes 속성을 사용한다.
	 */
	...
	// consume 속성값을 지정하지 않고 테스트
	// consume 속상값을 지정하지 않으면 기본값으로 "application/json" 미디어 타입으로 지정된다.
	...
	/*
	 *  7. Accept 매핑 사용
	 *  - produces 속성 값을 시정해서 요청하는 미디어 타입을 처리한다.
	 */
	
	
	// produces 속성 값을 지정하지 않고 테스트
	// produces 속상 값을 지정하지 않으면 기본값인 "application/json" 미디어 타입으로 지정된다.
	...
	public ResponseEntity<Board> readAccept(@PathVariable("boardNo") int boardNo){
		...
		ResponseEntity<Board> entiry = new ResponseEntity<Board>(board, HttpStatus.OK);
		return entiry;
	}
	

	
	// produces 속성 값에 "application"
