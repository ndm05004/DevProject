...
<body>
	...
	<p>
		...
		<font color="red">서버 쪽 컨트로러 메소드에서 @PathVariables를 사용하지 않는 경우 파라미터로 해당 값을 얻을 수 없다.</font>
	</p>
	...
	<p>5) HTML Form 필드가 여러개일 경우에 컨트롤러 매개변수의 위치는 상관없이 있는가?  </p>
	...
	<form action="/beans/register01" method="post">
		...
		password : <input type="text" name="password" value="1234"/><br>
		coin : <input type="text" name="coin" value="100"/><br>
		<input type="submit" value="registerJavaBenas01"/><br>
	</form>	<br/>
	
	<p>2) 폼 텍스트 필드 요소값을 자바빈즈 매개변수와 기본 데이터 타입인 정수 타입매개변수로 처리한다.</p>
	<form action="/beans/register02" method="post">
		userId : <input type="text" name="userId" value="hongkd"/><br>
		password : <input type="text" name="password" value="1234"/><br>
		coin : <input type="text" name="coin" value="100"/><br>
		<input type="submit" value="registerJavaBenas02"/><br>
	</form>	<br/>	
	...
	<form action="/beans/register03" method="post">
		...
		password : <input type="text" name="password" value="1234"/><br>
		coin : <input type="text" name="coin" value="100"/><br>
		<input type="submit" value="registerJavaBenas03"/><br>
	</form>	<br/>	
	...
	<h4>5. Date 타입처리</h4>	
	<hr/>
	<p>쿼리 파라미터(dateOfBirth=1234)로 전달받은 값이 문자열 형식에 맞지 않아 Date 타입으로 변환에 실패한다.</p>
	...
	<p>
		4) 쿼리 파라미터(dateOfBirth=2018/09/08)로 전달 받은 값이 날짜 문자열 형식으로 설정 시, Date 타입으로 받는가?
		<font color="green">SUCCESS</font>
	</p>
	<a href="/registerByGet01?userId=hongkd&dateOfBirth=2018/09/08">BUTTON07</a>
	
	
	<p>5) Member 매개변수와 쿼리 파라미터(dateOfBirth=20180908)로 전달받은 값이 문자열 형식으로 설정시, Date 타입으로 받는가?</p>
	<a href="/registerByGet02?userId=hongkd&dateOfBirth=20180908">BUTTON08</a>
	
	<p>6) Member 매개변수와 폼 방식 요청 전달받은 값이 날짜 문자열 형식으로 설정시, Date 타입으로 받는가?</p>
	...
	<p>4) 폼 라디오버튼 요소 값을 기본 데이터 타입인 문자열 타입 매개변수로 처리한다.</p>
	...
	<p>6) 복수 선택이 가능한 폼 셀렉트 박스 요소 값을 기본 데이터 타입인 문자열 타입 매개변수로 처리한다. </p>
	<form action="/registerMultiSelect" method="post">
		cars :<br/>
	...
	<p>5) 여러 개의 파일업로드를 폼 파일 요소값을 여러개의 MultipartFile 매개변수로 처리한다.</p>
	...
	<p> 6) 여러개의 파일업로드를 폼 파일 요소값을 MultipartFile 타입의 요소를 가진 리스트 컬렉션 타입매개변수로 처리한다.</p>
	...
	<p>8) 파일업로드 폼 파일 요소값과 텍스트 필드 요소값 MultipartFile 타입의 배열 매개변수로 처리한다.</p>
